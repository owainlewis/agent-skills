---
name: codex-run-loop
description: "Coordinates Codex worker threads or worktrees that turn one or more issues into draft PRs. Use when the user asks Codex to run an issue-to-PR loop, work through tickets, process agent-ready backlog items, run tickets in parallel, or produce draft PRs for review."
user-invocable: true
...
- When giving up on a ticket, remove its `agent:ready` label, add `needs:human`, and leave an issue
  comment explaining what went wrong, so the ticket lands with a human instead of being retried on
  the next run.
